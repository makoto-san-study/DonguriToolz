...
getOption('enabled_log_filter_self').then(enabled => { if(enabled == 'true') {
  ...
  const logs = rows.map(row => row.lastChild.textContent);
  const filter = {self: Array(rows.length).fill(true)};
  Promise.all([getOption('cannonlog_filter_self'), getOption('root_donguri')])
  .then(values => {
    ...
    function onChangeFilter(checked) {
      setOption('cannonlog_filter_self', checked.toString());
      if (checked) {
